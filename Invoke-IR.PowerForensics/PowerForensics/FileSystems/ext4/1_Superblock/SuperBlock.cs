...
namespace InvokeIR.PowerForensics.ext3
{
    public class Superblock
    {
        ...
        public readonly string AlgorithmBitmap;
        ...
        internal Superblock(byte[] bytes)
        {
            ...
            byte[] Part1Bytes = new byte[4];
            Array.Copy(VolumeUUIDBytes, 0x00, Part1Bytes, 0, Part1Bytes.Length);
            Array.Reverse(Part1Bytes);
            int Part1 = BitConverter.ToInt32(Part1Bytes, 0x00);

            byte[] Part2Bytes = new byte[2];
            Array.Copy(VolumeUUIDBytes, 0x04, Part2Bytes, 0, Part2Bytes.Length);
            Array.Reverse(Part2Bytes);
            short Part2 = BitConverter.ToInt16(Part2Bytes, 0x00);

            byte[] Part3Bytes = new byte[2];
            Array.Copy(VolumeUUIDBytes, 0x06, Part3Bytes, 0, Part3Bytes.Length);
            Array.Reverse(Part3Bytes);
            short Part3 = BitConverter.ToInt16(Part3Bytes, 0x00);

            byte[] Part4 = new byte[8];
            Array.Copy(VolumeUUIDBytes, 0x08, Part4, 0, Part4.Length);
            //Array.Reverse(Part4);
            VolumeUUID = new Guid(Part1, Part2, Part3, Part4);
            #endregion VolumeUUID
            
            #region VolumeLabel
            byte[] VolumeLabelBytes = new byte[16];
            Array.Copy(bytes, 0x78, VolumeLabelBytes, 0, VolumeLabelBytes.Length);
            VolumeName = Encoding.ASCII.GetString(VolumeLabelBytes).TrimEnd('\0');
            ...
            LastMountedDirectory = Encoding.ASCII.GetString(LastMountedDirectoryBytes).Split('\0')[0];
            #endregion LastMountedDirectory
            
            AlgorithmBitmap = ((S_ALGO_BITMAP)BitConverter.ToUInt32(bytes, 0xC8)).ToString();
